...
set "ABI=linux"
set "FLAGS="
call "%root_dir%\tools\builder.bat" "%script_dir%\linux\tests.c"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
call "%root_dir%\objcopy.bat" --strip-sections "%OUT%\%ARCH%-%ABI%_tests"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
...
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
call "%root_dir%\tools\builder.bat" "%script_dir%\freebsd\tests.c"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
call "%root_dir%\objcopy.bat" --strip-sections "%OUT%\%ARCH%-%ABI%_tests"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
...
if not "%ARCH%" == "riscv64" (
    ...
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\tools\builder.bat" "%script_dir%\windows\tests.exe.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
exit /b
...
if not defined NO_WASM32 (
    ...
    set "FLAGS_RELEASE=%opt% -s"
    set "FLAGS_DEBUG=-g"
    call "%root_dir%\tools\builder.bat" "%script_dir%\wasi\tests.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
