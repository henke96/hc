static void _curve25519_test(int32_t iterations, const uint8_t *k, const uint8_t *u, const uint8_t *expected) {
    uint8_t out[32];
    uint8_t nextK[32];
    uint8_t nextU[32];
    ...
    for (int32_t i = 0; i < iterations; ++i) {
        curve25519(&out[0], &nextK[0], &nextU[0]);
...
static void _curve25519_testEcdh(void) {
    uint8_t alicePriv[32] = { 0x77, 0x07, 0x6d, 0x0a, 0x73, 0x18, 0xa5, 0x7d, 0x3c, 0x16, 0xc1, 0x72, 0x51, 0xb2, 0x66, 0x45, 0xdf, 0x4c, 0x2f, 0x87, 0xeb, 0xc0, 0x99, 0x2a, 0xb1, 0x77, 0xfb, 0xa5, 0x1d, 0xb9, 0x2c, 0x2a };
    uint8_t bobPriv[32] = { 0x5d, 0xab, 0x08, 0x7e, 0x62, 0x4a, 0x8a, 0x4b, 0x79, 0xe1, 0x7f, 0x8b, 0x83, 0x80, 0x0e, 0xe6, 0x6f, 0x3b, 0xb1, 0x29, 0x26, 0x18, 0xb6, 0xfd, 0x1c, 0x2f, 0x8b, 0x27, 0xff, 0x88, 0xe0, 0xeb };
    uint8_t expectedShared[32] = { 0x4a, 0x5d, 0x9d, 0x5b, 0xa4, 0xce, 0x2d, 0xe1, 0x72, 0x8e, 0x3b, 0xf4, 0x80, 0x35, 0x0f, 0x25, 0xe0, 0x7e, 0x21, 0xc9, 0x47, 0xd1, 0x9e, 0x33, 0x76, 0xf0, 0x9b, 0x3c, 0x1e, 0x16, 0x17, 0x42 };

    uint8_t alicePub[32];
    uint8_t bobPub[32];
    curve25519(&alicePub[0], &alicePriv[0], &curve25519_ecdhBasepoint[0]);
    curve25519(&bobPub[0], &bobPriv[0], &curve25519_ecdhBasepoint[0]);

    uint8_t aliceShared[32];
    uint8_t bobShared[32];
    curve25519(&aliceShared[0], &alicePriv[0], &bobPub[0]);
    curve25519(&bobShared[0], &bobPriv[0], &alicePub[0]);

    CHECK(hc_MEMCMP(&aliceShared[0], &expectedShared[0], 32), RES == 0);
    CHECK(hc_MEMCMP(&bobShared[0], &expectedShared[0], 32), RES == 0);
}

static void curve25519_tests(void) {
