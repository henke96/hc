...
set "FLAGS_DEBUG=-g"
call "%root_dir%\tools\builder.bat" "%script_dir%\linux\tls.c"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
call "%root_dir%\objcopy.bat" --strip-sections "%OUT%\%ARCH%-%ABI%_tls"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
...
if not "%ARCH%" == "riscv64" (
    ...
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\tools\builder.bat" "%script_dir%\windows\tls.exe.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
exit /b
