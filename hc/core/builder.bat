...
if not defined NO_DEBUG (
    ...
    if defined ASSEMBLY (
        call "%root_dir%\core\cc.bat" -S -o "%OUT%\debug\%full_name%.s" %FLAGS_DEBUG% "%source%"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    )
    call "%root_dir%\core\cc.bat" -o "%OUT%\debug\%full_name%" %FLAGS_DEBUG% "%source%"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)

if defined ASSEMBLY (
    call "%root_dir%\core\cc.bat" -S -o "%OUT%\%full_name%.s" %FLAGS_RELEASE% "%source%"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
call "%root_dir%\core\cc.bat" -o "%OUT%\%full_name%" %FLAGS_RELEASE% "%source%"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b

set "analysis_flags=--analyze --analyzer-output text -Xclang -analyzer-opt-analyze-headers"
if not defined NO_ANALYSIS (
    call "%root_dir%\core\cc.bat" %analysis_flags% %FLAGS_RELEASE% "%source%"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\cc.bat" %analysis_flags% %FLAGS_DEBUG% "%source%"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
