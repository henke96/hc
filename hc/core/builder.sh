...
if test -z "$NO_DEBUG"; then
    ...
    if test -n "$ASSEMBLY"; then
        "$root_dir/core/cc.sh" -S -o "$OUT/debug/$full_name.s" "$@" "$source"
    fi
    "$root_dir/core/cc.sh" -o "$OUT/debug/$full_name" "$@" "$source"
fi

eval "set -- $FLAGS_RELEASE"
if test -n "$ASSEMBLY"; then
    "$root_dir/core/cc.sh" -S -o "$OUT/$full_name.s" "$@" "$source"
fi
"$root_dir/core/cc.sh" -o "$OUT/$full_name" "$@" "$source"

analysis_flags="--analyze --analyzer-output text -Xclang -analyzer-opt-analyze-headers"
if test -z "$NO_ANALYSIS"; then
    "$root_dir/core/cc.sh" $analysis_flags "$@" "$source"
    eval "set -- $FLAGS_DEBUG"
    "$root_dir/core/cc.sh" $analysis_flags "$@" "$source"
fi
