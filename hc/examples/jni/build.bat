...
setlocal
    ...
    if not defined NO_LINUX (
        ...
        call "%root_dir%\core\builder.bat" "%script_dir%\linux\lib%name%.so.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\objcopy.bat" --strip-sections "%OUT%\%ARCH%-%ABI%_lib%name%.so"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    )
    if not defined NO_FREEBSD (
        set "FLAGS=-fPIC -shared"
        call "%root_dir%\core\builder.bat" "%script_dir%\freebsd\lib%name%.so.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\objcopy.bat" --strip-sections "%OUT%\%ARCH%-%ABI%_lib%name%.so"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    )
    ...
    if not defined NO_WINDOWS (
        ...
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\builder.bat" "%script_dir%\windows\%name%.dll.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    )
exit /b
