...
build() {
    ...
    if test -z "$NO_LINUX"; then
        ...
        "$root_dir/core/builder.sh" "$script_dir/linux/lib$name.so.c"
        "$root_dir/core/objcopy.sh" --strip-sections "$OUT/$ARCH-${ABI}_lib$name.so"
    fi
    export ABI=freebsd14
    if test -z "$NO_FREEBSD"; then
        export FLAGS="-fPIC -shared"
        "$root_dir/core/builder.sh" "$script_dir/freebsd/lib$name.so.c"
        "$root_dir/core/objcopy.sh" --strip-sections "$OUT/$ARCH-${ABI}_lib$name.so"
    fi
    ...
    if test -z "$NO_WINDOWS"; then
        ...
        "$root_dir/core/builder.sh" "$script_dir/windows/$name.dll.c"
    fi
}
