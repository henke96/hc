...
setlocal
    ...
    if not defined NO_GNULINUX (
        ...
        call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\libc.so.6" "%root_dir%\core\hc\linux\gnu\libc.so.6.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\libdl.so.2" "%root_dir%\core\hc\linux\gnu\libdl.so.2.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\builder.bat" "%script_dir%\gnulinux\%name%.c"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    ...
    if not defined NO_WINDOWS (
        ...
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\genlib.bat" "%OUT%\user32.lib" "%root_dir%\core\hc\windows\dll\user32.def"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
        call "%root_dir%\core\genlib.bat" "%OUT%\gdi32.lib" "%root_dir%\core\hc\windows\dll\gdi32.def"
        if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
...
setlocal
    ...
    set "FLAGS_RELEASE=%opt%"
    set "FLAGS_DEBUG=-g"
    call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\libc.so" "%root_dir%\core\hc\linux\android\libc.so.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\liblog.so" "%root_dir%\core\hc\linux\android\liblog.so.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\libdl.so" "%root_dir%\core\hc\linux\android\libdl.so.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\cc.bat" -fPIC -shared -o "%OUT%\libandroid.so" "%root_dir%\core\hc\linux\android\libandroid.so.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\builder.bat" "%script_dir%\android\lib%name%.so.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
    call "%root_dir%\core\objcopy.bat" --strip-all "%OUT%\%ARCH%-%ABI%_lib%name%.so"
exit /b
...
if not defined NO_WASM32 (
    ...
    set "FLAGS_RELEASE=%opt% -s"
    set "FLAGS_DEBUG=-g"
    call "%root_dir%\core\builder.bat" "%script_dir%\web\%name%.wasm.c"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
