...
build() {
    ...
    if test -z "$NO_GNULINUX"; then
        ...
        "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/libc.so.6" "$root_dir/core/hc/linux/gnu/libc.so.6.c"
        "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/libdl.so.2" "$root_dir/core/hc/linux/gnu/libdl.so.2.c"
        "$root_dir/core/builder.sh" "$script_dir/gnulinux/$name.c"
    ...
    if test -z "$NO_WINDOWS"; then
        ...
        "$root_dir/core/genlib.sh" "$OUT/user32.lib" "$root_dir/core/hc/windows/dll/user32.def"
        "$root_dir/core/genlib.sh" "$OUT/gdi32.lib" "$root_dir/core/hc/windows/dll/gdi32.def"
...
build_android() {
    ...
    export FLAGS_RELEASE="$opt"
    export FLAGS_DEBUG="-g"
    "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/libc.so" "$root_dir/core/hc/linux/android/libc.so.c"
    "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/liblog.so" "$root_dir/core/hc/linux/android/liblog.so.c"
    "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/libdl.so" "$root_dir/core/hc/linux/android/libdl.so.c"
    "$root_dir/core/cc.sh" -fPIC -shared -o "$OUT/libandroid.so" "$root_dir/core/hc/linux/android/libandroid.so.c"
    "$root_dir/core/builder.sh" "$script_dir/android/lib$name.so.c"
    "$root_dir/core/objcopy.sh" --strip-all "$OUT/$ARCH-${ABI}_lib$name.so"
}
...
if test -z "$NO_WASM32"; then
    ...
    export FLAGS_RELEASE="$opt -s"
    export FLAGS_DEBUG="-g"
    "$root_dir/core/builder.sh" "$script_dir/web/$name.wasm.c"
