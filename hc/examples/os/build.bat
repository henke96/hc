...
set "FLAGS_RELEASE="
set "FLAGS_DEBUG="
call "%root_dir%\core\builder.bat" "%script_dir%\kernel\kernel.elf.c"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b

call "%root_dir%\core\objcopy.bat" -O binary "%OUT%\%ARCH%-%ABI%_kernel.elf" "%OUT%\kernel.bin"
if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
if not defined NO_DEBUG (
    call "%root_dir%\core\objcopy.bat" -O binary "%OUT%\debug\%ARCH%-%ABI%_kernel.elf" "%OUT%\debug\kernel.bin"
    if not errorlevel 0 ( exit /b ) else if errorlevel 1 exit /b
)
...
call "%root_dir%\core\builder.bat" "%script_dir%\bootloader\bootloader.efi.c"
