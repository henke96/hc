...
export FLAGS_DEBUG=
"$root_dir/core/builder.sh" "$script_dir/kernel/kernel.elf.c"

"$root_dir/core/objcopy.sh" -O binary "$OUT/$ARCH-${ABI}_kernel.elf" "$OUT/kernel.bin"
if test -z "$NO_DEBUG"; then
    "$root_dir/core/objcopy.sh" -O binary "$OUT/debug/$ARCH-${ABI}_kernel.elf" "$OUT/debug/kernel.bin"
fi
...
"$root_dir/core/builder.sh" "$script_dir/bootloader/bootloader.efi.c"
