...
static void _sha1_blocks(uint32_t *state, const uint8_t *in, int64_t numBlocks) {
    uint32_t w[80];
    uint32_t r[85];
...
static void sha1_update(struct sha1 *self, const uint8_t *in, int64_t size) {
...
static void sha1_finish(struct sha1 *self, uint8_t *hash) {
